...
function getPublicationCount(locale?: string): number {
  ...
  const entries = bibtexParse.toJSON(bibtexContent) as Array<{ entryTags: Record<string, string> }>;
  return entries.filter((entry) => !isPublicationHidden(entry.entryTags)).length;
}
