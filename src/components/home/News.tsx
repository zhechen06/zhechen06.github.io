...
const newsMarkdownComponents = {
    ...
    a: ({ ...props }) => (
        <a
            ...
            className="text-ui-accent underline decoration-ui-accent/30 underline-offset-4 transition-colors rounded hover:bg-accent/10 hover:decoration-ui-accent/70"
        />
    ),
    strong: ({ children }: React.ComponentProps<'strong'>) => <strong className="font-semibold text-ui-accent">{children}</strong>,
                ...
                {visibleItems.map((item, index) => (
                    <div key={index} className="flex items-start gap-3">
                        <span className="mt-0.5 w-[4.75rem] flex-shrink-0 whitespace-nowrap text-sm tabular-nums text-ui-muted">
	                        ...
	                        <div className="min-w-0 text-base leading-relaxed text-justify text-ui-body">
                            <p>
                                {item.tag && (
