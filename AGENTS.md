...
| CV | `public/cv/CV-Zhe-Chen-English.tex`, `public/cv/CV-Zhe-Chen-Chinese.tex` | `public/cv/` | `/cv`, `/zh/cv` |
...
- Publications: `content/publications.bib` is the source of truth. Store names as `Family, Given`, join people with `and`, keep `author` free of role markers, and list corresponding authors in a separate `corresponding = {Family, Given and ...}` field. For new publications or metadata changes, use `skills/homepage-publications/SKILL.md`, check Zotero when a title/DOI is available and Zotero is running, then run `npm run cv:publication-sync-check`. If title, authors, venue, year, DOI, `corresponding`, `sciif`, or ordering changes affect the CV, update both CV TeX files, apply the CV ordering rules, and regenerate both PDFs.
...
- Treat these groups as exclusive: first-author papers stay in the first-author group even if another author is corresponding; non-first-author papers where `Chen, Zhe` appears in the BibTeX `corresponding` field go in the corresponding-author group before ordinary second-author papers.
