...
- Normalize publication titles to sentence case across the website and CV: capitalize the first word, the first word after a colon, proper nouns, product names, and acronyms; do not copy publisher-wide Title Case for every major word.
- Store every person name as `Family, Given` and join names with BibTeX `and`. Keep role markers out of `author`; store corresponding authors separately as `corresponding = {Family, Given and ...}` using names that exactly match entries in `author`.
- Decide whether the change is CV-visible. Titles, authors, corresponding-author metadata, venue, year, DOI, status, `sciif`, and any change that affects CV ordering require CV inspection and usually PDF regeneration. Site-only fields such as `preview`, `description`, `keywords`, and `selected` do not require CV edits by themselves.
...
- Fill the site-specific fields:
  ...
  - `selected`: `{true}` only when the paper should appear on the homepage selected list; omit it otherwise
  - `hidden`: `{true}` only when a paper should be temporarily excluded from all website publication surfaces and CV sync checks; omit it to show the paper
  - `accepted`: acceptance date in `YYYY-MM-DD` when it is backed by the PDF first page or publisher evidence
- Add a name to `corresponding` only when the publisher page or PDF explicitly marks that person as a corresponding author. Use `and` for multiple corresponding authors and do not append `*` to names in `author`.
...
- If Zotero includes a local PDF attachment, inspect the first pages before adding corresponding-author metadata or other provenance-sensitive details:
...
- After changing name parsing or author metadata conventions, also run:

```bash
npm run publication:name-tests
```
...
- Summarize the publication entries changed, evidence used for any corresponding-author metadata, assets added or reused, CV sync outcome, verification commands run, and whether the deployment/static-export build was intentionally skipped or run.
